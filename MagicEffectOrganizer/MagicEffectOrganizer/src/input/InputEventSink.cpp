...
namespace MEO_INPUT
{
	...
	void EventSink::Register()
	{
		...
		if (!inputMgr || !sink) {
			LOG_WARN("MagicEffectOrganizer: Failed to register InputEvent sink");
			return;
		}

		inputMgr->AddEventSink(sink);
		g_registered = true;
		LOG_INFO("MagicEffectOrganizer: InputEvent sink registered");
	}
	...
	RE::BSEventNotifyControl EventSink::ProcessEvent(RE::InputEvent* const* a_event, RE::BSTEventSource<RE::InputEvent*>*)
	{
		...
		for (auto* input = *a_event; input; input = input->next) {
			...
			if (cfg.enabled && cfg.hotkey == keyCode) {
				if (!manager->HideCurrentlySelectedMagicMenuEffect()) {
					LOG_INFO("MagicEffectOrganizer: Hotkey pressed but no hideable selected effect was resolved");
				}
			}
		}
