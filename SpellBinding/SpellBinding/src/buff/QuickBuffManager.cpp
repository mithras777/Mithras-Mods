...
namespace QUICK_BUFF
{
	...
	void Manager::Serialize(SKSE::SerializationInterface* a_intfc) const
	{
		...
		if (!a_intfc->OpenRecord(kBindingRecord, kSerializationVersion)) {
			return;
		}
		...
		writeString(m_config.combatStart.spellFormID);
		writeString(m_config.combatEnd.spellFormID);
		writeString(m_config.healthBelow70.spellFormID);
		writeString(m_config.healthBelow50.spellFormID);
		writeString(m_config.healthBelow30.spellFormID);
		writeString(m_config.crouchStart.spellFormID);
		writeString(m_config.sprintStart.spellFormID);
		writeString(m_config.weaponDraw.spellFormID);
		writeString(m_config.powerAttackStart.spellFormID);
		writeString(m_config.shoutStart.spellFormID);
	}
	...
	void Manager::Deserialize(SKSE::SerializationInterface* a_intfc)
	{
		...
		while (a_intfc->GetNextRecordInfo(type, version, length)) {
			if (type != kBindingRecord || version != kSerializationVersion) {
				continue;
			}
			...
			readString(m_config.combatStart.spellFormID);
			readString(m_config.combatEnd.spellFormID);
			readString(m_config.healthBelow70.spellFormID);
			readString(m_config.healthBelow50.spellFormID);
			readString(m_config.healthBelow30.spellFormID);
			readString(m_config.crouchStart.spellFormID);
			readString(m_config.sprintStart.spellFormID);
			readString(m_config.weaponDraw.spellFormID);
			readString(m_config.powerAttackStart.spellFormID);
			readString(m_config.shoutStart.spellFormID);
		}
		ClampConfig(m_config);
		ResetRuntime();
	}
	...
	void Manager::LoadConfig()
	{
		...
		if (cfgNode.contains("version") && cfgNode["version"].is_number_unsigned()) {
			cfg.version = cfgNode["version"].get<std::uint32_t>();
		}
		...
		if (cfgNode.contains("triggers") && cfgNode["triggers"].is_object()) {
			const auto& t = cfgNode["triggers"];
