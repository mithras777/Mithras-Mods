...
namespace CONSOLE::COMMANDS {
	...
	void Plugin(RE::Script* a_script)
	{
		...
		if (command.compare("apply") == 0) {
			AUTOMCM::Runtime::GetSingleton().ForceApplyNow();
			consoleLog->Print("AutoMCM: apply requested");
			return;
		}
		if (command.compare("reset") == 0) {
			AUTOMCM::Runtime::GetSingleton().ResetAll();
			consoleLog->Print("AutoMCM: reset requested");
			return;
		}
		if (command.compare("dump") == 0) {
			const auto desired = AUTOMCM::ProfileStore::GetSingleton().GetDesiredEntries();
			consoleLog->Print("AutoMCM: %d desired entries tracked", static_cast<int>(desired.size()));
			return;
		}
