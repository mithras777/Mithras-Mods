...
namespace SHO_INPUT
{
	...
	void EventSink::Register()
	{
		...
		if (!inputMgr || !sink) {
			LOG_WARN("ShoutOrganizer: Failed to register InputEvent sink");
			return;
		}

		inputMgr->AddEventSink(sink);
		g_registered = true;
		LOG_INFO("ShoutOrganizer: InputEvent sink registered");
	}
	...
	RE::BSEventNotifyControl EventSink::ProcessEvent(RE::InputEvent* const* a_event, RE::BSTEventSource<RE::InputEvent*>*)
	{
		...
		for (auto* input = *a_event; input; input = input->next) {
			...
			if (cfg.enabled && cfg.hotkey == keyCode) {
				if (!manager->HideCurrentlySelectedMagicMenuPowerOrShout()) {
					LOG_INFO("ShoutOrganizer: Hotkey pressed but no hideable selected power/shout was resolved");
				}
			}
		}
